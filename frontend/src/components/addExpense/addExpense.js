...
const AddExpense = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await fetch(`http://localhost:4000/api/v1/add-expense`, {
                method: "POST",
    ...
    return (
        <div className='flex flex-col space-y-6 font-mono bg-purple-500/30 px-10 py-8 rounded-xl mt-[40dvh]'>
            <h2 className='text-4xl font-semibold'> Add Expense</h2>
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                ...
                <button type="submit" className='bg-black text-white px-4 py-2 hover:scale-110 duration-300 rounded-lg'>Add Expense</button>
            </form>
        </div>
    );
};
